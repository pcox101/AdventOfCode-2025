...
for (let row = 0; row < gameboard.length - 1; row++)
{
    for (let col = 0; col < gameboard[row].length; col++) {
        if ((gameboard[row][col] != '.') && (gameboard[row][col] != '^'))
        {
            ...
            if (d1 == '^') {
                part1++;
                updateOneBelow(row, col - 1, cVal);
                updateOneBelow(row, col + 1, cVal)
            }
            else {
                updateOneBelow(row, col, cVal);
            }
        }
    }
}
...
function updateOneBelow(currentRow: number, newCol: number, cVal: number) {
    let l = gameboard[currentRow + 1][newCol];
    let lVal = cVal;
    if (l != '.') {
        lVal = lVal + parseInt(l);
    }
    gameboard[currentRow + 1][newCol] = lVal.toString();
}
